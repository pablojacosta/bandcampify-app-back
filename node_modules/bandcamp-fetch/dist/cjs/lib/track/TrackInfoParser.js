...
const AlbumInfoParser_1 = __importDefault(require("../album/AlbumInfoParser"));
class TrackInfoParser {
    static parseInfo(html, opts) {
        ...
        if (opts.trackUrl) {
            const { path: trackUrlPath, hash: trackUrlHash } = (0, Parse_1.splitUrl)(opts.trackUrl);
        ...
        if (!rawBasic || !rawExtra) {
            throw new Parse_1.ParseError('Failed to parse track info: data missing \'ld_json\' or \'tralbum\' fields.', html);
        }
        let basic, extra;
        try {
            ...
            throw new Parse_1.ParseError('Failed to parse track info: JSON error in basic data.', rawBasic, error);
        }
        try {
            extra = JSON.parse(rawExtra);
        }
        catch (error) {
            throw new Parse_1.ParseError('Failed to parse track info: JSON error in extra data.', rawExtra, error);
        }
        if (!basic || typeof basic !== 'object') {
            throw new Parse_1.ParseError('Failed to parse track info: invalid basic data');
        }
        if (!extra || typeof extra !== 'object') {
            throw new Parse_1.ParseError('Failed to parse track info: invalid extra data');
        }
        const track = {
            type: 'track',
            name: basic.name,
        ...
        const imageUrl = (0, Parse_1.reformatImageUrl)(basic.image, opts.albumImageFormat);
        if (imageUrl) {
        ...
        const duration = (0, Parse_1.getAdditionalPropertyValue)(basic, 'duration_secs');
        ...
        const publisher = (0, Parse_1.parsePublisher)(basic, opts.artistImageFormat);
        if (publisher) {
        ...
        const label = (0, Parse_1.parseLabelFromBackToLabelLink)($);
        if (label) {
            track.label = {
                name: label.name,
                url: label.url
...
_a = TrackInfoParser, _TrackInfoParser_parseTrackInfoFromAlbum = function _TrackInfoParser_parseTrackInfoFromAlbum(html, opts, trackPosition) {
    const album = AlbumInfoParser_1.default.parseInfo(html, opts);
    ...
    throw new Parse_1.ParseError(`Track not found in album (position: ${trackPosition}`, html);
};
