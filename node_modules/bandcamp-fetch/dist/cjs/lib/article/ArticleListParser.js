...
class ArticleListParser {
    static parseList(html, opts) {
        ...
        const dailyUrl = Constants_1.URLS.DAILY;
        const result = {
            articles: [],
            total: 0,
            start: 0,
        ...
        $('articles-list').each((i, list) => {
            $('.list-article', $(list)).each((i, article) => {
                ...
                if (category && categoryUrl) {
                    category.url = (0, Parse_1.isAbsoluteUrl)(categoryUrl) ? categoryUrl : (0, Parse_1.normalizeUrl)(categoryUrl, dailyUrl);
                }
                // Date
                infoTextCategoryLink.remove();
                infoTextMiddot.remove();
                const date = (0, Parse_1.stripLineBreaks)(infoText.text()).trim();
                // Title and url
                const titleLink = article.find('a.title');
                const title = titleLink.text();
                let url = titleLink.attr('href');
                if (!(0, Parse_1.isAbsoluteUrl)(url)) {
                    url = (0, Parse_1.normalizeUrl)(url, dailyUrl);
                }
                if (titleLink) {
                    const parsed = {
                        url,
                    ...
                    const imageUrl = (0, Parse_1.reformatImageUrl)(imgSrc, opts.imageFormat);
                    if (imageUrl) {
        ...
        const resultsText = (0, Parse_1.stripLineBreaks)($('#num-results').text()).trim();
