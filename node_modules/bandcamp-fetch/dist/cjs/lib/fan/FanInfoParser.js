...
class FanInfoParser {
    static parseInfo(html, opts) {
        ...
        try {
            ...
            throw new Parse_1.ParseError('Failed to parse fan info: JSON error in data-blob.', html, error);
        }
        const fanData = parsed.fan_data || {};
        const fanId = fanData.fan_id;
        if (!fanId || !fanData.name || !fanData.username) {
            throw new Parse_1.ParseError('Failed to parse fan info: invalid data.', html);
        }
        const result = {
            type: 'fan',
