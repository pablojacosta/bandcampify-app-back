...
class AlbumInfoParser {
    static parseInfo(html, opts) {
        ...
        if (!rawBasic || !rawExtra) {
            throw new Parse_1.ParseError('Failed to parse album info: data missing \'ld_json\' or \'tralbum\' fields.', html);
        }
        let basic, extra;
        try {
            ...
            throw new Parse_1.ParseError('Failed to parse album info: JSON error in basic data.', rawBasic, error);
        }
        try {
            extra = JSON.parse(rawExtra);
        }
        catch (error) {
            throw new Parse_1.ParseError('Failed to parse album info: JSON error in extra data.', rawExtra, error);
        }
        if (!basic || typeof basic !== 'object') {
            throw new Parse_1.ParseError('Failed to parse album info: invalid basic data');
        }
        if (!extra || typeof extra !== 'object') {
            throw new Parse_1.ParseError('Failed to parse album info: invalid extra data');
        }
        const artist = {
        ...
        if (Array.isArray(basic.albumRelease)) {
            const releases = basic.albumRelease.filter((release) => release.musicReleaseFormat).map((release) => {
                ...
                const releaseUrl = (0, Parse_1.normalizeUrl)(release['@id'], album.url);
                if (releaseUrl) {
                ...
                if (release.image) {
                    ...
                    const releaseImageArtId = (0, Parse_1.getAdditionalPropertyValue)(release, 'art_id');
        ...
        if (Array.isArray(extra.trackinfo)) {
            ...
            const tracks = extra.trackinfo.map((track) => {
                ...
                const trackUrl = (0, Parse_1.normalizeUrl)(track.title_link, album.url);
                if (trackUrl) {
