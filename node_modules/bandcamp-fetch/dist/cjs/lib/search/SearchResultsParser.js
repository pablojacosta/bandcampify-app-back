...
class SearchResultsParser {
    static parseResults(html, opts) {
        ...
        resultsList.each((i, resultListItem) => {
            ...
            const imageUrl = (0, Parse_1.reformatImageUrl)(imgSrc, resultType === 'album' || resultType === 'track' ? opts.albumImageFormat : opts.artistImageFormat);
            ...
            resultInfo.find('.subhead, .genre, .tags, .released, .length').each((i, info) => {
                ...
                if (info.hasClass('subhead')) {
                    if (resultType === 'artist' || resultType === 'label') {
                        ...
                        artist = (0, Parse_1.substrAfter)(infoText, 'by ')?.trim();
                        if (resultType === 'track') {
                            album = (0, Parse_1.substrBefore)(infoText, ' by');
                            if (album) {
                                album = (0, Parse_1.substrAfter)(album, 'from ')?.trim();
                            }
                        }
                    }
                    return true;
                ...
                if (info.hasClass('genre')) {
                    genre = (0, Parse_1.substrAfter)(info.text(), 'genre: ')?.trim();
                    return true;
                }
                if (info.hasClass('tags')) {
                    tags = (0, Parse_1.substrAfter)(info.text(), 'tags:');
                    if (tags) {
                        tags = (0, Parse_1.stripLineBreaks)((0, Parse_1.stripMultipleWhitespaces)(tags)).trim();
                    }
                    return true;
                }
                if (info.hasClass('released')) {
                    releaseDate = (0, Parse_1.substrAfter)(info.text(), 'released ')?.trim();
                    return true;
                }
                if (info.hasClass('length')) {
                    ...
                    const numTracksStr = tracksText ? (0, Parse_1.substrBefore)(tracksText, 'tracks') : null;
                    if (numTracksStr) {
                        numTracks = parseInt(numTracksStr, 10);
                    }
                    const durationStr = minutesText ? (0, Parse_1.substrBefore)(minutesText, 'minutes') : null;
                    if (durationStr) {
...
_a = SearchResultsParser, _SearchResultsParser_matchSearchItemTypeWithResult = function _SearchResultsParser_matchSearchItemTypeWithResult(searchType, resultType) {
    if (searchType === SearchAPI_1.SearchItemType.All && VALID_RESULT_TYPES.includes(resultType)) {
        return true;
    }
    switch (searchType) {
        case SearchAPI_1.SearchItemType.ArtistsAndLabels:
            return resultType === 'artist' || resultType === 'label';
        case SearchAPI_1.SearchItemType.Albums:
            return resultType === 'album';
        case SearchAPI_1.SearchItemType.Tracks:
            return resultType === 'track';
        case SearchAPI_1.SearchItemType.Fans:
